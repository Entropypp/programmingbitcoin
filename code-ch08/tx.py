...
class Tx:
    ...
    def verify_input(self, input_index):
        ...
        if script_pubkey.is_p2sh_script_pubkey():
            ...
            redeem_script_raw = tx_in.script_sig.cmds[-1]
            # prepend the length of the RedeemScript using encode_varint
            redeem_script_len = encode_varint(len(redeem_script_raw))
            redeem_script = redeem_script_len + redeem_script_raw
            # parse the RedeemScript
            redeem_script = Script.parse(BytesIO(redeem_script))             
        else:
            redeem_script = None
